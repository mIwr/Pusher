...
import 'package:pusher/model/bit_bool.dart';
...
class AppProperties
{
  ...
  static Future<AppProperties> loadPropertiesAsync() async
  {
    ...
    var flags = new BitBool(_bitFlagsDefaultValue);
    var langCode = "";

    if (!preferences.containsKey(kPrefFlagsKey)) {
      await preferences.setInt(kPrefFlagsKey, flags.rawFlags);
    }
    flags = new BitBool(preferences.getInt(kPrefFlagsKey) ?? _bitFlagsDefaultValue);
    ...
    return new AppProperties(flags: flags, nativePrefsSource: preferences);
  }
}