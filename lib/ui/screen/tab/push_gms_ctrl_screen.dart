...
class _PushGmsCtrlScreen extends State<PushGmsCtrlScreen> {
  ...
  void initState() {
    super.initState();
    final selectedProj = gmsController.selected;
    if (selectedProj != null) {
      _selectedProjIdNotifier.value = selectedProj.id;
    }
    _projKeys = gmsController.generateOrderedList().map((e) => e.id).toList();
    _projectsUpdListener = gmsController.onProjectsCollectionUpdate.listen(_onProjectsUpd);
    _selectedProjUpdListener = gmsController.onSelectedProjUpdate.listen(_onSelectedProjUpd);
    _targetsUpdListener = gmsController.onProjectTargetsCollectionUpdate.listen(_onProjTargetsUpd);
    _targetUpdListener = gmsController.onTargetUpdate.listen(_onTargetUpd);
  }
  ...
  void _onProjTargetsUpd(MapEntry<GmsProjectConfig, HashMap<String, PushTargetType>> upd) {
    ...
    upd.value.forEach((key, value) {
      if (value == PushTargetType.token) {
        tokens.add(MapEntry(key, upd.key.id));
      } else {
        topics.add(MapEntry(key, upd.key.id));
      }
    });
    if (tokens.isEmpty) {
  ...
  void _onTargetUpd(ControllerUpdateEvent<MapEntry<String, PushTargetType>> upd) {
    if (upd.type == ControllerUpdateEventType.deleted) {
      if (upd.item.value == PushTargetType.token) {
        PushTargetsDbUtil.deleteDeviceTokens(servicePrefix: PushTargetsDbUtil.kGmsServicePrefix, ids: [upd.item.key]);
      } else {
        PushTargetsDbUtil.deletePushTopics(servicePrefix: PushTargetsDbUtil.kGmsServicePrefix, ids: [upd.item.key]);
      }
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(body: SafeArea(child: GestureDetector(onTap: () {
      ...
      SingleChildScrollView(physics: BouncingScrollPhysics(), padding: EdgeInsets.symmetric(horizontal: 16), child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
        Padding(padding: EdgeInsets.fromLTRB(0, 16, 0, 8), child: Text(S.current.push_ctrl_panel_gms_project, style: primaryTextTheme.displaySmall)),
        Stack(alignment: Alignment.centerLeft, children: [
          DropdownButtonHideUnderline(child: Padding(padding: EdgeInsets.only(right: 144), child: DropdownButton2<String>(value: gmsController.selected?.id,
              ...
              customButton: Container(height: kAppBtnHeight, padding: EdgeInsets.symmetric(horizontal: 16), decoration: BoxDecoration(color: currColorScheme.secondaryContainer, borderRadius: BorderRadius.circular(12)), child: Stack(alignment: Alignment.centerLeft, children: [
                Padding(padding: EdgeInsets.only(right: 32), child: ValueListenableBuilder<String?>(valueListenable: _selectedProjIdNotifier, builder: (context, val, child) {
              ...
              items: _projKeys.map((item) => DropdownMenuItem(value: item, child: Text(item, style: primaryTextTheme.labelLarge))).toList(growable: false), onChanged: (selected) {
                if (selected == null) {
          ...
          Align(alignment: Alignment.centerRight, child: Row(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.center, children: [
            SizedBox(width: 40, height: 40,
                child: TextButton(style: TextButton.styleFrom(backgroundColor: currColorScheme.secondaryContainer, foregroundColor: currColorScheme.tertiary,
                  ...
                  Navigator.of(context).push(MaterialPageRoute(settings: Screens.projCtor.routeSettings(), builder: (context) => GmsPushProjectCtorScreen()));
        ...
        Stack(alignment: Alignment.centerLeft, children: [
          ...
          Align(alignment: Alignment.centerRight, child: SizedBox(width: 40, height: 40,
              child: TextButton(style: TextButton.styleFrom(backgroundColor: currColorScheme.secondaryContainer, foregroundColor: currColorScheme.tertiary,
                ...
                final id = gmsController.selected?.id;
                if (id != null) {
        ...
        Row(crossAxisAlignment: CrossAxisAlignment.center, children: [
          ...
          }, child:  Text(S.current.push_ctrl_panel_gms_validate_only_hint, style: primaryTextTheme.bodyMedium)))
        ]),
        Padding(padding: EdgeInsets.only(top: 16)),
      ...
      Align(alignment: Alignment.bottomCenter, child: Padding(padding: EdgeInsets.fromLTRB(16, 0, 16, 12), child: SizedBox(width: MediaQuery.of(context).size.width,
          child: TextButtonAccent(content: S.current.general_send, onPressed: () async {
            ...
            final proj = gmsController.selected;
            if (proj == null) {
